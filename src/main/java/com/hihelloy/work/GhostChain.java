...
public class GhostChain extends ChiAbility implements AddonAbility {
    ...
    private enum ChainState { THROWING, BOUND, PULLING, MISSED }
    ...
    private void setFields() {
        ...
        this.pullSpeed = config.getDouble("ExtraAbilities.Hihelloy.Chi.GhostChain.PullSpeed", 0.55);
        ...
        int nodeCount = (int) (throwRange * 4);
        Vector scale = new Vector(0.07, throwRange / nodeCount, 0.07);
        this.rope = new VerletRope(this.vh, player, this.hookLocation, throwRange, nodeCount, 1,
                Color.fromRGB(180, 180, 200), 1.0f, true, true, scale, scale, Material.IRON_BLOCK);
        ...
        this.hookObject = new GameObject(player, GameObject.DisplayMode.BLOCK_DISPLAY,
                this.hookLocation, new Vector(0.2, 0.2, 0.2), new Vector(0, 0, 0), new Vector());
    ...
    public void progress() {
        ...
        if (state == ChainState.THROWING) {
            ...
            if (boundTarget == null || boundTarget.isDead()) { remove(); return; }
            ...
            if (player.getLocation().distanceSquared(boundTarget.getLocation()) < 4) { remove(); return; }
            ...
            Vector toPlayer = player.getEyeLocation().toVector()
                    .subtract(boundTarget.getLocation().toVector());
            ...
            Vector pullVel = toPlayer.normalize().multiply(pullSpeed);
            boundTarget.setVelocity(pullVel);
            ...
            vh.update();
            vh.display();
            if (rope.getRopeLength() <= 0.5) { remove(); return; }
        }

        hookObject.setLocation(hookLocation);
        hookObject.setRotation(new Vector(0, Math.toRadians(hookYaw), Math.toRadians(hookYaw * 0.5)));
        hookObject.updateAndDisplay();
    }
    ...
    private void beginPull() {
        MovementHandler handler = MovementHandler.getFromEntityAndAbility(boundTarget, this);
    ...
    public void load() {
        ...
        ConfigManager.getConfig().addDefault("ExtraAbilities.Hihelloy.Chi.GhostChain.PullSpeed", 0.55);
